...
import type { ConversationSnapshot, ConversationSummary, RuntimeStatus } from "@shared/types";
...
export class ConversationService {
    ...
    async createConversation() {
        ...
        const sessionManager = SessionManager.create(conversationWorkspaceDir, this.globalConfig.sessionsDir, {
            id: conversationId
        });
    ...
    private async createManagedSession(conversationRecord: ConversationRecord, sessionManager: SessionManager,) {
            ...
            switch (event.type) {
                ...
                case "message_start":
                    ...
                    if (message.role === 'assistant') {
                        ...
                        const content: (TextContent | ImageContent)[] = typeof message.content === "string" ? [{ type: "text", text: message.content }] : message.content;
    ...
    private async ensureManagedSession(conversationId: string) {
        ...
        if (existsSync(restoredSessionFile)) {
            sessionManager = SessionManager.open(restoredSessionFile, this.globalConfig.sessionsDir, conversationRecord.workspaceDir);
        } else {
            sessionManager = SessionManager.create(conversationRecord.workspaceDir, this.globalConfig.sessionsDir, {
                id: conversationId
            });
        }
